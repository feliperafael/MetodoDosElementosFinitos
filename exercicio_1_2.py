...
C2 = (np.exp(-1/np.sqrt(epsilon)) -1)/(np.exp(1/np.sqrt(epsilon)) - np.exp(-1/np.sqrt(epsilon)))
C1 = -1 - C2
def du(x):
    return 1#( 1 -(-C1/np.sqrt(epsilon))*np.exp(-x/np.sqrt(epsilon)) + (C2/np.sqrt(epsilon))*np.exp(x/np.sqrt(epsilon)))/epsilon
    return (-C1/np.sqrt(epsilon))*np.exp(-x/np.sqrt(epsilon)) + (C2/np.sqrt(epsilon))*np.exp(x/np.sqrt(epsilon))
    return 4*np.pi*np.pi*np.sin(2*np.pi*x)
def exata(x):
    return (C1*np.exp(-x/np.sqrt(epsilon)) + C2*np.exp(x/np.sqrt(epsilon)) + 1)
...
def solucao_aproximada(nint,grau,nel):
        ...
        for l in range(nint): #loop em ponto de gaus
            ...
            for i in range(grau+1):
                ...
                for j in range(grau+1):
                    ...
                    ke_l[i,j] += phi_function(i,p[l])*phi_function(j,p[l])*w[l] # termo reativo
        print ke
        print fe
        #multiplico por 
        fe = fe*(h/2.0)
    ...
    print("\nSolucao :",solucao)
    print("exata :",exata(x))
    plot(x,solucao,"exercicio_02_"+str(nel)+"_episilon_"+str(epsilon),nel)
    #plot(np.linspace(inicio, fim, num=10000),exata(np.linspace(inicio, fim, num=10000)))
    print F
    print K
    return solucao
