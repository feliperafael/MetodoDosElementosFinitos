    ...
    for elemento in range(nel): #loop em elementos
        ...
        for l in range(nint): #loop em ponto de gaus
            ...
            for i in range(grau+1):
                ...
                for j in range(grau+1):
                    ...
                    ke_aux[i,j] += ((epsilon)*w[l]* dphi_function(i,p[l]) * dphi_function(j,p[l])\
                    + beta*phi_function(i,p[l])*dphi_function(j,p[l])*w[l] \
                    -  du(0.5*(x[ii]+x[ii+grau] + p[l]*h))\
                    )*beta*phi_function(i,p[l])*dphi_function(j,p[l])#SUPG

                    ke_l[i,j] += beta*phi_function(i,p[l])*dphi_function(j,p[l])*w[l] # termo reativo
                    print("ke_aux : ",ke_aux[i][j])
        print ke
        print fe
        #multiplico por 
        fe = fe*(h/2.0)
        ...
        for i in range(grau+1):
            for j in range(grau+1):
                ...
                #else:              
                K[elemento+i][elemento+j] += ke[i][j] + ke_l[i][j] + ke_aux[i][j]
